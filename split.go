...
func splitPhrase(t *Trie, phrase string, sep string) string {
	...
	*/
	...
	unmatched := 0
	for i := 0; i < len(phrase); i += len(phrase[i : i+1]) {
...
func splitPhraseMinEntropy(t *Trie, phrase string, sep string, reverse bool) string {
	...
	*/
	...
	bestWords := []string{}
	unmatched := 0
	for u := 0; u < len(phrase); u += len(phrase[u : u+1]) {
		fmt.Println(string(phrase[u : u+1]))
...
func main() {
	dict := readDictionary("chula5000.txt")
	for i := range dict {
		w := strings.Split(dict[i], " ")[0]
		dict[i] = strings.Trim(strings.ToLower(w), " \n,.")
	}
	var trie *Trie = buildTrie(dict, false)
	fmt.Println(splitPhraseMinEntropy(trie,
		"77/1 หมู่12 อาคาร ศูนย์วัฒนธรรมจีนยูนนานบ้านสันติชล ถนนสันติชล ตำบลเวียงใต้ อำเภอปาย",
		" ", false))
	fmt.Println(findLongestWord(trie, "ธนาคารพาณิชย์ธนาคารพาณิชย์"))
}
